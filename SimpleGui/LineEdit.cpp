...
////////////////////////////////////////////////////////////
void LineEdit::handleEvent(sf::Event const& event, sf::Window& window)
{
    if(isEnabled() && isVisible())
    {
        if(isActive())
        {
            ...
            if(Event::mouseOut(getPosition(),sf::Vector2f(mSprite.getGlobalBounds().width,mSprite.getGlobalBounds().height),window) && Event::clickLeft(event))
                deactivate();
            if(Event::returnKey(event) && mCallbacks[0])
                mCallbacks[0]();
        }
        else if(!isActive() && Event::mouseIn(getPosition(),sf::Vector2f(mSprite.getGlobalBounds().width,mSprite.getGlobalBounds().height),window) && Event::clickLeft(event))
            activate();
