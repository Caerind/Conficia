...
////////////////////////////////////////////////////////////
void Slider::handleEvent(sf::Event const& event, sf::Window& window)
{
    if(isVisible() && isEnabled())
    {
        if(Event::mouseIn(getPosition(),sf::Vector2f(mSprite.getGlobalBounds().width,mSprite.getGlobalBounds().height),window))
        {
            ...
            if(sf::Mouse::isButtonPressed(sf::Mouse::Left))
            {
                ...
                if(mUseAxisX)
                {
                    newPos.x = static_cast<float>(sf::Mouse::getPosition(window).x) - getPosition().x;
                ...
                {
                    newPos.y = static_cast<float>(sf::Mouse::getPosition(window).y) - getPosition().y;
