...
const std::string Compress::base64_table = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                                              "abcdefghijklmnopqrstuvwxyz"
                                              "0123456789+/";

bool Compress::base64_encode(std::string& data)
{
    int count = 0;
...
bool Compress::base64_decode(std::string& data)
{
    int count = 0;
...
bool Compress::decompressString(std::string& data)
{
    z_stream zstream;
...
bool Compress::compressString(std::string& data)
{
...
bool Compress::uncompress(std::string& line)
{
    std::string defaultLine = line;
    if (!Compress::base64_decode(line))
    {
        line = defaultLine;
        return false;
    }
    if (!Compress::decompressString(line))
    {
        line = defaultLine;
        return false;
    }
...
bool Compress::compress(std::string& line)
{
    std::string defaultLine = line;
    ...
    if (!Compress::base64_encode(line))
    {
        line = defaultLine;
        return false;
    }
